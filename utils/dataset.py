...
class MultimodalityDataset(Dataset):
    ...
    def process_raw_data(self, min_date, max_date):
        ...
        ids, sents, audios, tables, y_three, y_seven, y_fifteen, y_thirty = [], [], [], [], [], [], [], []
        ...
        for corp in tqdm(all_corps):
            ...
            try:
                ...
                three_days, seven_days, fifteen_days, thirty_days = float(data_slice['after_3_days_vol']), \
                    float(data_slice['after_7_days_vol']), float(data_slice['after_15_days_vol']), float(data_slice['after_30_days_vol'])
                all_ten_retx = []
                for i in range(1, 11):
        ...
        y_fifteen = torch.FloatTensor(y_fifteen)
        y_thirty = torch.FloatTensor(y_thirty)

        return (ids, sents, audios, tables, y_three, y_seven, y_fifteen, y_thirty)
